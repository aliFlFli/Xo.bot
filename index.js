...
const levelKeyboard = {
    inline_keyboard: [
        [
            {
                text: '🍃 آسان',
                callback_data: 'lvl_easy'
            }
        ],
        [
            {
                text: '⚙️ معمولی',
                callback_data: 'lvl_normal'
            }
        ],
        [
            {
                text: '🔥 سخت',
                callback_data: 'lvl_hard'
            }
        ]
    ]
};
...
async function poll() {
    ...
    try {
        ...
        for (const upd of res.data.result) {
            ...
            console.log('📥 آپدیت جدید');

            await handleUpdate(upd);

            if (
                upd.update_id > lastOffset
            ) {
